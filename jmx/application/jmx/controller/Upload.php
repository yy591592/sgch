...
class Upload extends Common
{
    ...
    public function upload()
    {
        ...
        if ($info) {
            ...
            $result['url'] = DOMAIN . $upload_path . $path;
            $result['index'] = rand();
            return $result;
        } else {
            // 上传失败获取错误信息
    ...
    public function file()
    {
        ...
        $upload_path = '../uploads/' . $type . '/file/';
        $fileKey = array_keys(request()->file());
        // 获取表单上传文件 例如上传了001.jpg
        $file = request()->file($fileKey['0']);
        // 移动到框架应用根目录/public/uploads/ 目录下
        $info = $file->validate(['ext' => 'zip,rar,pdf,swf,ppt,psd,ttf,txt,xls,doc,docx,apk'])->move($upload_path);
        if ($info) {
            $result['code'] = 1;
            $result['info'] = '文件上传成功!';
            $path = str_replace('\\', '/', $info->getSaveName());
            $result['url'] = $upload_path . $path;
    ...
    public function video()
    {
        ...
        $file = request()->file($fileKey['0']);
        // 移动到框架应用根目录/public/uploads/ 目录下
        $info = $file->validate(['ext' => 'swf,flv,mp3,mp4,wav,wma,wmv,mid,avi,mpg,asf,rm,rmvb'])->move("." . $upload_path);
        if ($info) {
            $result['code'] = 1;
            $result['info'] = '视频上传成功!';
            $path = str_replace('\\', '/', $info->getSaveName());
